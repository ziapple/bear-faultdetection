...
def build_model(input_shape=(cwru_input_emd.TIME_PERIODS,), num_classes=cwru_input_emd.LABEL_SIZE):
    ...
    model.add(LSTM(filters=32, kernel_size=128, strides=2, input_shape=input_shape, activation='relu', padding='same'))
    model.add(Dropout(rate=0.25))
    model.add(MaxPooling1D(pool_size=2))
    model.add(LSTM(filters=64, kernel_size=64, activation='relu', padding='same'))
    model.add(Dropout(0.25))
    model.add(MaxPooling1D(pool_size=2))
    model.add(LSTM(filters=128, kernel_size=32, activation='relu', padding='same'))
    model.add(Dropout(0.25))
    model.add(MaxPooling1D(pool_size=2))
